#!/usr/bin/env python3
"""Assemble the v9 cut into outputs/preview_v9.mp4.
...
CUT = [
    ...
    (V9 / "t0_onelife2.mp4", 0, None, False, "omen"),
    (V9 / "a6_sign3.mp4", 0, None, False, "arcade"),
    ...
    (V9 / "a_hard2.mp4", 0, None, False, None),
    # --- Bronze ---
    (V8 / "b1_wake4.mp4", 0, None, False, "silence"),
    ...
    (V9 / "b6_scream3.mp4", 0, None, False, None),
    ...
    (V8 / "b10_forty2.mp4", 0, None, False, None),
    (V3 / "b12_cough.mp4", 0, None, False, None),        # same cough, death #2
    (V9 / "w1c.mp4", 0, None, False, None),         # respawn: "...Again."
    (V6 / "b13_cry2.mp4", 0, None, False, None),
    (V3 / "b14_three_lives.mp4", 0, None, False, None),
    (V9 / "w2c.mp4", 0, None, False, None),          # respawn: playing dead
...
FREEZE = {"r3_gesture_q": 1.5}  # tail-freeze: hold the last frame N extra seconds
...
for i, (path, ts, td, mute, era) in enumerate(CUT):
    ...
    fc.append(f"[{i}:v]trim=start={ts}:end={ts + d0:.3f},setpts=PTS-STARTPTS,{tpad}"
...
for k, (era, s, t) in enumerate(spans):
    ...
    fc.append(f"[{idx}:a]aresample=48000,atrim=duration={span_d:.3f},"
              f"afade=t=in:d={fade_in:.3f},afade=t=out:st={max(0, span_d - 3):.3f}:d=3,"
...
graph_file = ROOT / "outputs/assemble_v9_graph.txt"
...
subprocess.run(["ffmpeg", "-y", "-v", "error"] + inputs
               + ["-filter_complex_script", str(graph_file),
                  ...
                  str(ROOT / "outputs/preview_v9.mp4")], check=True)
print("wrote outputs/preview_v9.mp4")
