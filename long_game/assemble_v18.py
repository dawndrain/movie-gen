#!/usr/bin/env python3
"""v18: THE cut (credits ending promoted per Dawn). Cuts mid-second exit attempt,
...
V14 = ROOT / "outputs/video14"
V16 = ROOT / "outputs/video16"
...
CUT = [
    ...
    (V14 / "t0_longgame.mp4", 0, None, False, "omen"),  # approved "...Huh." tail spliced back
    ...
    (V12 / "a_hard_fix.mp4", 0, None, False, None),  # dubbed "Cass" kept — cutaway reverted (accent/continuity)
    # --- Bronze ---
    ...
    (V14 / "b7_name3_bronze.mp4", 0, None, False, None),  # histogram-matched to bronze palette
    (V14 / "b8_leave2.mp4", 0, None, False, None),     # retake: pan follows the point, old woman visible
    ...
    (V13 / "r9_acid3.mp4", 0, None, False, None),    # Dawn prefers this take
    ...
    (V14 / "p2_board4.mp4", 0, None, False, None),   # only ONE silver-haired man
    ...
    (V12 / "q2_fix.mp4", 0, None, False, None),      # reverted to dubbed freeze-cover take
    (V16 / "q3_walk2.mp4", 0, None, False, None),    # on-model Deshawn/Milo, forward walk
    (V6 / "q4_real2.mp4", 0, None, False, "omen_q4"),
    (V4 / "q5_nested.mp4", 0, None, False, "silence"),
    (V4 / "q6_longtime2.mp4", 0, None, False, None),
    (V6 / "q7_fail2.mp4", 0, 7.5, False, "silence"),   # plays dry; dip to black at the cut
    (ROOT / "outputs/credits.mp4", 0, None, False, "silence"),  # amber CRT credits: hum + ominous arcade cue
]
...
# Ambience beds: steady-state scene textures under everything (never event-y —
# Seedance's own sparse ambience stays the foreground). Spans keyed by clip stem:
# bed runs from the START of first_stem to the START of end_stem.
AMB = ROOT / "ambience"
# gains calibrated per bed: ElevenLabs beds have wildly different native levels
# (-19 to -64 dB mean); each gain lands the bed at ~-34 dB mix mean (featured:
# baths/workshop/arcade) or ~-38 dB (the rest), i.e. under all dialogue.
AMBIENCE = [
    # Beds loudnorm'd to -30 LUFS, EQ'd (highpass 100 / lowpass 7k kills the
    # ElevenLabs HF whine). Volume = dB offset; film dialogue anchor -22.4 LUFS.
    # Gemini-auditioned (listen.py): all PASS except workshop (busy fire pops)
    # which rides the quiet tier. roman_city + library dropped: 4 candidates
    # failed audition; native clip audio carries those scenes.
    ("a6_sign2",   "a_ski3",      "amb_arcade_n.m4a",   "-12dB"),
    ("a8_out3",    "b1_wake5",    "amb_arcade_n.m4a",   "-12dB"),
    ("b1_wake5",   "b21_party3",  "amb_bronze_n.m4a",   "-16dB"),  # ends before the party
...
GRADE = {"q7_fail2": "fade=t=out:st=6.7:d=0.8,"}  # gentle fade to black into credits
AFADE = {"q7_fail2": (6.9, 0.6)}  # audio eases out with the picture
...
graph_file = ROOT / "outputs/assemble_v18_graph.txt"
...
subprocess.run(["ffmpeg", "-y", "-v", "error"] + inputs
               + ["-filter_complex_script", str(graph_file),
                  ...
                  str(ROOT / "outputs/previews/preview_v18.mp4")], check=True)
print("wrote outputs/previews/preview_v18.mp4")
