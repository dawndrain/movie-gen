#!/usr/bin/env python3
"""Assemble the ANIMATIC (stills + ElevenLabs cast) into mewtwo/outputs/preview_v1.mp4.
Music: scene cues per the treatment; silence under most dialogue."""
import subprocess
from pathlib import Path

HERE = Path(__file__).parent
V1 = HERE / "outputs/animatic_clips"
V1DUB = HERE / "outputs/animatic_clips"
MUS = HERE / "music"
...
def pick(name):
    dirs = (V1DUB, V1) if name in ADOPT_DUB else (V1,)
    for d in dirs:
...
graph = HERE / "outputs/assemble_animatic_graph.txt"
...
subprocess.run(["ffmpeg", "-y", "-v", "error"] + inputs
               + ["-filter_complex_script", str(graph),
                  ...
                  str(HERE / "outputs/animatic_v1.mp4")], check=True)
print("wrote mewtwo/outputs/animatic_v1.mp4")
